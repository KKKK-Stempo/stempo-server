 ...
 *
 * 필수:
 *   deployContainer : 새 컨테이너 이름 (String)
 *   image           : 이미지 이름 (String)
 *
 * 옵션:
 *   hostPort      - 호스트 포트 (지정하지 않으면 -p 옵션 미포함)
 *   containerPort - 컨테이너 포트 (기본값: "8080")
 *   imageTag      - 도커 이미지 태그 (선택, 지정하지 않으면 deployContainer 값 사용)
 *   network       - 애플리케이션 네트워크 (지정하지 않으면 --network 옵션 미포함)
 *   restartPolicy - 재시작 정책 (예: "always", "on-failure" 등; 지정하지 않으면 미포함)
 *   extraVolumes  - 볼륨 옵션 리스트 (각 항목은 "hostPath:containerPath" 형식의 문자열; 자동으로 "-v " 접두사 추가)
 *   extraEnv      - 환경 변수 옵션 리스트 (각 항목은 "KEY=VALUE" 형식의 문자열; 자동으로 "-e " 접두사 추가)
 *   extraNetworks - 추가로 연결할 네트워크 (단일 문자열 또는 리스트)
 *   extraArgs     - docker run 명령어에 추가할 기타 옵션 (List)
 *
 * docker run 예시:
 *   docker run -d --name <deployContainer> -p <hostPort>:<containerPort> [--network <network>] [--restart <restartPolicy>]
 *       [ <볼륨옵션> ] [ <환경변수옵션> ] [ <extraArgs>... ] <image>:<imageTag>
 */
def deployNewInstance(Map params = [:]) {
    if (!params.deployContainer || !params.image) {
        error "deployNewInstance (blue-green): 'deployContainer'와 'image' 파라미터는 필수입니다."
    }
    ...
    sh """
        ...
        if docker ps --filter 'name=${params.deployContainer}' --format '{{.Names}}' | grep -q '${params.deployContainer}'; then
            docker stop ${params.deployContainer}
            docker rm ${params.deployContainer}
        fi
    """

    // 이미지 태그: imageTag 파라미터가 제공되면 사용, 없으면 deployContainer 사용
    def tag = params.imageTag ?: params.deployContainer
    cmd += " ${params.image}:${tag}"

    // docker run 명령어 동적 구성
    def cmd = "docker run -d --name ${params.deployContainer}"
    ...
    if (params.extraVolumes && params.extraVolumes instanceof List) {
        def volumeStr = params.extraVolumes.collect { vol ->
            vol.trim().startsWith("-v") ? vol.trim() : "-v " + vol.trim()
        }.join(" ")
        cmd += " " + volumeStr
    }
    if (params.extraEnv && params.extraEnv instanceof List) {
        def envStr = params.extraEnv.collect { envVar ->
    ...
    if (params.extraNetworks) {
        if (params.extraNetworks instanceof List) {
            params.extraNetworks.each { net ->
                ...
                if (netCheck == 0) {
                    sh "docker network connect ${net} ${params.deployContainer}"
                } else {
            ...
            if (netCheck == 0) {
                sh "docker network connect ${params.extraNetworks} ${params.deployContainer}"
            } else {
